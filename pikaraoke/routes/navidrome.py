...
@navidrome_bp.route("/navidrome_stream/<song_id>", methods=["GET"])
def navidrome_stream(song_id: str):
    """Proxy the audio stream for a Navidrome song, keeping credentials server-side.

    Audio is transcoded to 128kbps via Navidrome to keep bandwidth low for remote clients.
    """
    k = get_karaoke_instance()
    ...
    try:
        ...
        logging.error("Navidrome stream error: %s", e)
        return jsonify({"error": "Could not stream from Navidrome"}), 502
